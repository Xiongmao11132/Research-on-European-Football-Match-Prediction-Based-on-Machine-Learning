...
# 参数分布
param_grid = {
    'max_depth': [9],  # 控制树的最大深度，None 表示不限制深度
    'min_samples_split': [90],  # 分支时节点中至少包含的样本数
    'min_samples_leaf': [12],  # 每个叶节点最少的样本数
    'max_features': [ None],  # 分裂时考虑的最大特征数
    'criterion': ['entropy'],  # 划分质量的衡量标准
    'class_weight': [None],  # 控制类别不平衡的加权
    'splitter': ['best']
}
...
# 保存模型
joblib.dump(best_model, 'DecisionTree_model.pkl')
...
joblib.dump(train_columns, 'train_columns_DT.pkl')
...
train_columns = joblib.load('train_columns_DT.pkl')
...
# # 模型选择和训练
# model = DecisionTreeClassifier(max_depth=10, min_samples_split=10, min_samples_leaf=2, random_state=1)
# model.fit(X_train, y_train)
