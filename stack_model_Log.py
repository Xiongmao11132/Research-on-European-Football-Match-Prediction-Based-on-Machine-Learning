...
meta_model = LogisticRegression(max_iter=1000000, random_state=1, n_jobs=-1)  # 启用并行性
...
joblib.dump(stack_model, 'stack_model_stack_Log.pkl')
...
joblib.dump(train_columns, 'train_columns_stack_Log.pkl')
...
train_columns = joblib.load('train_columns_stack_Log.pkl')
X_new = X_new.reindex(columns=train_columns, fill_value=0)

# 使用堆叠模型预测新数据
stack_model = joblib.load('stack_model_stack_Log.pkl')
...
plt.title('混淆矩阵')
plt.show()
plt.savefig("Stack_model_Log.png")
