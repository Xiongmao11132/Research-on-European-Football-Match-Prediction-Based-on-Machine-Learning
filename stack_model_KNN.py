...
meta_model = KNeighborsClassifier(n_jobs=-1)  # 添加并行性
...
joblib.dump(stack_model, 'stack_model_stack_KNN.pkl')
...
joblib.dump(train_columns, 'train_columns_stack_KNN.pkl')
...
train_columns = joblib.load('train_columns_stack_KNN.pkl')
X_new = X_new.reindex(columns=train_columns, fill_value=0)

# 使用堆叠模型预测新数据
stack_model = joblib.load('stack_model_stack_KNN.pkl')
...
plt.title('混淆矩阵')
plt.show()
plt.savefig("Stack_model_KNN.png")
